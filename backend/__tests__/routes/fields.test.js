...
describe("Fields routes", () => {
    ...
    describe("POST /fields/add", () => {
        it("should add fields succesfully", async () => {
            const mockRows = [{ id: 1, name: 'Field Name', description: 'Some description', location: 'somewhere', price_per_hour: 50, image_path: '/uploads/fields/test-image.jpg' }];
    ...
    describe("POST /book/:fieldId/:slot_id", () => {
        it("should return that the slot is booked", async () => {
            ...
            const res = await request(app).post(`/fields/book/${fieldId}/${slot_id}`).query({ date });
        ...
        it("should make the reservation but fail to send the email", async () => {
            ...
            expect(res.body.message).toBe("Reservation created but failed to send confirmation email");
            expect(sendReservationConfirmation).toHaveBeenCalledWith('test@example@gmail.com', 'name of field', date, 'name of slot');
        });
